...
from models.models import normalized_mutual_information, Dim_redu
from models.models import normalized_conditional_mutual_information
import random
...
def execute_modulation(args, model, device, dataloader, log_name, epoch):
    ...
    with torch.no_grad():
        ...
        for step, (image, spec, label, index) in tqdm(enumerate(dataloader)):
            ...
            for i, item in enumerate(label):
                ...
                feature_v = out_v[i]
                feature = out[i]
                fea_a = Dim_redu(feature_a)
                fea_v = Dim_redu(feature_v)
                fea = Dim_redu(feature)
                nmi_a = normalized_mutual_information(fea_a, fea)
                nmi_v = normalized_mutual_information(fea_v, fea)
                cnmi_a = normalized_conditional_mutual_information(fea, fea_a, fea_v)
                cnmi_v = normalized_conditional_mutual_information(fea, fea_v, fea_a)
                ii_a = nmi_a - cnmi_a
                ii_v = nmi_v - cnmi_v
                ...
                if index_all == label[i]:
                    value_all = all[0][index_all]
                if index_v == label[i]:#p(y_hat)
                    value_v = v[0][index_v]
                if index_a == label[i]:
                    value_a = a[0][index_a]

                contrib_a = value_all * nmi_a + value_a * ii_v
                contrib_v = value_all * nmi_v + value_v * ii_a  
                if nmi_a < nmi_v:
                    contribution_lb[int(index[i])] = value_all*nmi_v
                else: 
                    contribution_lb[int(index[i])] = value_all*nmi_a
                contribution[int(index[i])] = (contrib_a, contrib_v)

    train_dataloader = None
    # print(contribution.keys())
    if epoch >= args.warmup - 1:
        train_dataset = AV_KS_Dataset_MI(
    ...
    return contribution_lb, contribution, train_dataloader
...
def main():
    ...
    if args.train:
        ...
        for epoch in range(args.epochs):
            ...
            if epoch >= args.warmup - 1:
                conlb, con, train_dataloader = execute_modulation(
                    args, model, device, train_val_dataloader, log_name, epoch
                )
            else:
                conlb, con,  _ = execute_modulation(
                    args, model, device, train_val_dataloader, log_name, epoch
                )
