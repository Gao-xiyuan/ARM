...
class ClsHead(BaseModule):
    ...
    def train_predict(
        ...
        # The part can not be traced by torch.fx
        predictions = []
        predictions.append(self._get_train_predictions(cls_scores[0], data_samples))
        predictions.append(self._get_train_predictions(cls_scores[1], data_samples))
        predictions.append(self._get_train_predictions(cls_scores[2], data_samples))
        return predictions

    def _get_train_predictions(self, cls_score, data_samples):
        ...
        for i in range(len(target)):
            pred_gt_scores.append(float(pred_scores[i][target[i]]))

        return sum(pred_gt_scores)/len(target)
